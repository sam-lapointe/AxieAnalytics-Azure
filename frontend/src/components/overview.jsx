    ...
    return (
        <div className="grid grid-rows-2 gap-6 m-5 p-6 border-2 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 col-span-full">
                <OverviewLineChart data={data} keyValue="sales" label="Total Sales" labelData={data["total_sales"]} keyName="Axies Sold"/>
                <OverviewLineChart data={data} keyValue="volume_eth" label={insert_eth_logo("Total Volume")} labelData={data["total_volume_eth"]} keyName={<img src={eth_logo} className="w-3 h-3 my-auto"/>} />
                <OverviewLineChart data={data} keyValue="avg_price_eth" label={insert_eth_logo("Average Price")} labelData={data["avg_price_eth"]} keyName={<img src={eth_logo} className="w-3 h-3 my-auto"/>} />
            </div>
        </div>
    )
}