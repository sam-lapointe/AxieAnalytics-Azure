...
class Transaction:
    ...
    async def process_logs(self, transaction_hash) -> list:
        ...
        """
        try:
            logging.info(f"[process_logs] Processing logs for transaction {transaction_hash}...")
            ...
            weth_transfer_signature_hash = weth_contract.get_event_signature_hash("Transfer")
            ...
            axie_transfer_signature_hash = axie_proxy_contract.get_event_signature_hash("Transfer")
            ...
            for log in logs:
                logging.info(f"[process_logs] Log Contract Address: {Web3.to_checksum_address(log['address'])}, Topic Event: 0x{log['topics'][0].hex()}")
                topic = f"0x{log['topics'][0].hex()}"
                
                if Web3.to_checksum_address(log["address"]) == weth_contract_address and topic == weth_transfer_signature_hash:
                    """
                    ...
                    """
                    logging.info(f"[process_logs] Retrieving data for WETH transfer...")
                    ...
                    if event_data["args"]["_to"] == recipient:
                        ...
                        weth_value = Web3.from_wei(event_data["args"]["_value"], "ether")
                        prices.append(float(weth_value))
                        logging.info(f"[process_logs] Found WETH transfer going to the recipient of the transaction. Added WETH value of {weth_value} to list of prices.")
                elif Web3.to_checksum_address(log["address"]) == axie_proxy_contract_address and topic == axie_transfer_signature_hash:
                    """
                    ...
                    logging.info(f"[process_logs] Found Axie transfer. Added Axie {axie_id} to the list of axies.")
            ...
            logging.error(f"[process_logs] An unexpected error occured while processing logs for transaction {transaction_hash}: {e}")
            raise e