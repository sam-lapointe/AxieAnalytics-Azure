...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('0x8c5be1e5ebec7d5bd14f71443d9b2b6e3d3e8e8e8e8e8e8e8e8e8e8e8e8e8e8', 'Approval'),
    ]
)
def test_get_event_name(mocker, abi, topic_hash, expected_event_name):
    ...
    implementation_contract = Contract(conn, w3, '0xabcdefabcdefabcdefabcdefabcdefabcdefabcd')
    ...
    implementation_contract._Contract__contract = implementation_contract._Contract__w3.eth.contract(
...
def test_get_event_data(mocker, abi):
    ...
    log_data = AttributeDict(
        {
            ...
            'topics': [
                ...
                HexBytes('0x000000000000000000000000fff9ce5f71ca6178d3beecedb61e7eff1602950e'),  # _from
                HexBytes('0x000000000000000000000000094300dacf0eed244664e326d27406f0b90b8809')  # _to
            ],
            'data': HexBytes('0x00000000000000000000000000000000000000000000000000063f54c04cbcfb'),  # _value
            'blockNumber': 44153279,
            'transactionHash': HexBytes('0xb05e64ab435371a5c4b6e23f416a37fec881419228db0e35d9b3549204f549eb'),
            'transactionIndex': 1,
            'blockHash': HexBytes('0xf95b5c3227fc15c4c882f8287b490b99e629dc31dd015da2b7a9d6d4b0ee0f93'),
            'logIndex': 4,
            'removed': False
        }
    )
    ...
    transfer_topic_hash = '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef'
    event_data_result = proxy_contract.get_event_data(transfer_topic_hash, log_data)
    print(event_data_result)

    assert event_data_result["args"]["_from"] == "0xffF9Ce5f71ca6178D3BEEcEDB61e7Eff1602950E"
    assert event_data_result["args"]["_to"] == "0x094300DacF0eED244664e326d27406F0B90b8809"
...
def test_get_event_signature_hash(mocker, abi, event_name, expected_signature_hash):
    ...
    if event_name == "Transfer":
        event_signature_hash_result = proxy_contract.get_event_signature_hash(event_name)
